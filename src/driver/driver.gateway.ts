  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit
{
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      this.logger.log(
        `Driver client connected: ${client.id} - Driver: ${driverId}`,
      );
  ...
  async handleLocationUpdate(
    ...
    try {
      ...
      if (!this.isValidCoordinate(latitude, longitude)) {
        this.logger.warn(
          `Driver ${driverId} sent invalid coordinates: ${latitude}, ${longitude}`,
        );
        return;
      }

      // Update driver's location using ProfileService
      await this.profileService.updateLocation(driverId, {
        latitude,
        longitude,
      });
  ...
  async handleNavigationUpdate(
    ...
    try {
      ...
      const timeToPickup = Math.max(
        payload.remainingTime || 0,
        payload.timeToNextDestinationSeconds || 0,
      );
      const distanceToPickup = Math.max(
        payload.remainingDistance || 0,
        payload.distanceToNextDestinationMeters || 0,
      );
      const timeToDrop = Math.max(
        timeToPickup,
        payload.timeToFinalDestinationSeconds || 0,
      );
      const distanceToDrop = Math.max(
        distanceToPickup,
        payload.distanceToFinalDestinationMeters || 0,
      );
      ...
      if (!cachedDataStr) {
        ...
        kmTravelled = 0;
        this.logger.log(
          `Booking ${payload.bookingId}: First navigation update, initial distance: ${Math.floor(initialDistanceToPickup / 1000)}km`,
        );
      } else {
        ...
        if (cachedData.bookingId !== payload.bookingId) {
          ...
          kmTravelled = 0;
          this.logger.log(
            `Booking ${payload.bookingId}: New booking started, initial distance: ${Math.floor(initialDistanceToPickup / 1000)}km`,
          );
        } else {
          // Same booking - calculate distance travelled
          initialDistanceToPickup =
            cachedData.initialDistanceToPickup || distanceToPickup;
      ...
      this.logger.log(
        `Driver ${driverId} | Booking ${payload.bookingId}: ${kmTravelled}km travelled`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to process navigation update for driver ${driverId}:`,
        error,
      );
    }
  }
