...
export class GstService {
  ...
  async addGstDetails(
    userId: string,
    createGstDetailsDto: CreateGstDetailsDto,
    ...
    if (existingGst) {
      ...
      if (!existingGst.isActive && existingGst.customerId === userId) {
        throw new BadRequestException(
          'GST number already exists but is inactive. Please reactivate it.',
        );
      }
  ...
  async updateGstDetails(
    userId: string,
    id: string,
    updateGstDetailsDto: UpdateGstDetailsDto,
  ) {
