...
import {
  ...
  NonAgriculturalProductDto,
  PackageDetailsDto,
  PersonalProductDto,
  UpdatePackageDetailsDto,
} from 'src/booking/dtos/package.dto';

// Merge existing Package model + partial update into a complete PackageDetailsDto
export function mergeUpdateToPackageDto(
  existing: Package,
  updateDto: UpdatePackageDetailsDto,
): PackageDetailsDto {
  ...
  if (merged.productType === ProductType.AGRICULTURAL) {
    ...
    if (!(updateDto.nonAgricultural instanceof NonAgriculturalProductDto)) {
      throw new BadRequestException(
        'Non-agricultural product details required',
      );
    }
...
export function toPackageDetailsDto(pkg: Package): PackageDetailsDto {
  ...
  if (pkg.productType === ProductType.AGRICULTURAL) {
    if (!pkg.productName || !pkg.gstBillUrl)
      throw new BadRequestException('Agricultural product details required');
  ...
  // Handle NON_AGRICULTURAL product type (commercial)
  if (!pkg.gstBillUrl || !pkg.bundleWeight)
  ...
  const nonAgricultural: NonAgriculturalProductDto = {
    ...
    numberOfProducts: pkg.numberOfProducts
      ? Number(pkg.numberOfProducts)
      : undefined,
    packageDimensions:
...
export function toPackageCreateData(
  dto: PackageDetailsDto,
...
export function toPackageUpdateData(
  updateDto: UpdatePackageDetailsDto,
): Prisma.PackageUpdateInput {
  const out: Prisma.PackageUpdateInput = {};

  if (updateDto.productType !== undefined)
    out.productType = updateDto.productType;

  // Handle PERSONAL product type
  if (updateDto.productType === ProductType.PERSONAL) {
    if (updateDto.personal) {
      if (updateDto.personal.productName !== undefined)
        out.productName = updateDto.personal.productName;
    }
    ...
    if (updateDto.approximateWeight !== undefined)
      out.approximateWeight = updateDto.approximateWeight;
    if (updateDto.weightUnit !== undefined)
      out.weightUnit = updateDto.weightUnit;
    if (updateDto.packageImageUrl !== undefined)
      out.packageImageUrl = updateDto.packageImageUrl;
    if (updateDto.transportDocUrls !== undefined)
      out.transportDocUrls = updateDto.transportDocUrls;
  ...
  if (updateDto.agricultural) {
    if (updateDto.agricultural.productName !== undefined)
      out.productName = updateDto.agricultural.productName;
    if (updateDto.agricultural.gstBillUrl !== undefined)
      out.gstBillUrl = updateDto.agricultural.gstBillUrl;
  ...
  if (updateDto.nonAgricultural) {
    ...
    if (na.numberOfProducts !== undefined)
      out.numberOfProducts = na.numberOfProducts;
    if (na.packageDimensions) {
      if (na.packageDimensions.length !== undefined)
        out.length = na.packageDimensions.length;
      if (na.packageDimensions.width !== undefined)
        out.width = na.packageDimensions.width;
      if (na.packageDimensions.height !== undefined)
        out.height = na.packageDimensions.height;
      if (na.packageDimensions.unit !== undefined)
        out.dimensionUnit = na.packageDimensions.unit;
    }
    if (na.packageDescription !== undefined)
      out.description = na.packageDescription;
