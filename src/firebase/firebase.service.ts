...
export class FirebaseService implements OnModuleInit {
    ...
    try {
      ...
      if (serviceAccountPath) {
        ...
        const serviceAccountKey = {
          type:
            this.configService.get<string>('FIREBASE_TYPE') ||
            'service_account',
          project_id:
  ...
  async verifyGoogleIdToken(
    ...
    try {
      const ticket = await this.googleClient.verifyIdToken({
        ...
        audience:
          this.configService.get<string>('GOOGLE_CLIENT_ID') ||
          '691159300275-37gn4bpd7jrkld0cmot36vl181s3tsf3.apps.googleusercontent.com',
      });
  ...
  async notifyAllSessions(
    ...
    const sessions = await this.sessionService.findSessionsByUserId(
      userId,
      userType,
    );
    ...
    const tokenSet = new Set(
      sessions.filter((s) => s.fcmToken).map((s) => s.fcmToken!),
    );
    ...
    await this.sessionService.updateSessionsByUserId(userId, userType, {
